...
export default function LogMacrosSummaryCurrent({
	children,
	compactMode = false,
	detailedMode = false,
	useSkeleton = false,
	...
	const pieData: PieItemType[] = [
		{
			name: 'Carbs',
			value: logData?.macros.totalCarbs ?? 0,
			fill: 'var(--color-carb)'
		},
		{
			name: 'Protein',
			value: logData?.macros.totalProtein ?? 0,
			fill: 'var(--color-protein)'
		},
		{
			name: 'Fat',
			value: logData?.macros.totalFat ?? 0,
			fill: 'var(--color-fat)'
		}
	];

	const currentData = {
		calories: logData?.macros.caloriesConsumed ?? 0,
	...
	currentData.totalGrams =
		currentData.carbs + currentData.fat + currentData.protein;
